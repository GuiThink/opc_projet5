...
class Filters(object):
    """
    Prepares datas by applying some filters
    """
    def __init__(self, raw_data):
        self.raw_data = raw_data

        # category filtering process
        self.category_keepers = self.filter_categories(self.raw_data)
        self.ready_categories = sorted(self.remove_duplicates(self.category_keepers))

        # product filtering process
        self.ready_products = sorted(self.filter_products(self.raw_data, self.category_keepers))
        #
        # # store filtering process
        self.store_keepers = self.filter_stores(self.raw_data, self.category_keepers)
        self.ready_stores = sorted(self.remove_duplicates(self.store_keepers))

    def filter_categories(self, raw_data):
        """
    ...
    def remove_duplicates(self, list_to_clean):
        """
        Removes duplicates from a given list
        """
        return set(list_to_clean)

    def filter_products(self, raw_data, keepers):
        """
    ...
    def filter_stores(self, raw_data, keepers):
        """
